...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .neumorphic-card { box-shadow: none !important; border: 1px solid #ddd !important; border-radius: 0 !important; margin: 0 !important; }
            .main-content { padding: 0 !important; margin: 0 !important; }
            .container { max-width: 100% !important; width: 100% !important; margin: 0 !important; }
            .timeline::before { display: none; }
            .timeline-item::before { border: 1px solid #000; }
        }
    </style>
</head>
                    ...
                    <div class="col-md-8">
                        <div class="neumorphic-card p-5 text-center">
                            ...
                            <form action="" method="GET" class="d-flex gap-2 mx-auto justify-content-center" style="max-width: 500px;">
                                <input type="text" name="id" class="form-control neumorphic-input w-100 fs-5 text-center fw-bold" placeholder="e.g. C-XXXX-XXXX" required>
                                <button type="submit" class="btn neumorphic-btn btn-primary fw-bold px-4">Search</button>
                            </form>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-11">
                            <div class="d-flex justify-content-between align-items-end mb-4 px-2">
                                <div>
                                    <span class="text-muted text-uppercase fw-bold smaller letter-spacing-1">Ref ID: #<?= escape($shipment['id']) ?></span>
                            ...
                            <div class="row g-4">
                                <div class="col-md-7">
                                    <div class="neumorphic-card p-4 h-100">
                                        <div class="d-flex align-items-center mb-4 pb-3 border-bottom border-secondary border-opacity-10">
                                            <h5 class="fw-bold mb-0 flex-grow-1"><i class="bi bi-box-seam me-2 text-primary"></i>Shipment Details</h5>
                                            <?php 
                                            ...
                                            ?>
                                            <span class="badge-neumorphic <?= $status_class ?> px-4 py-2 fw-bold"><?= escape($shipment['status']) ?></span>
                                        </div>

                                        <div class="row g-4">
                                            <div class="col-sm-6">
                                                <div class="text-muted smaller fw-bold mb-1 uppercase text-uppercase">From (Origin)</div>
                                                <div class="fw-bold fs-5"><?= escape($shipment['origin_city']) ?></div>
                                            </div>
                                            <div class="col-sm-6 text-sm-end">
                                                <div class="text-muted smaller fw-bold mb-1 uppercase text-uppercase">To (Destination)</div>
                                                <div class="fw-bold fs-5"><?= escape($shipment['dest_city']) ?></div>
                                            </div>
                                            
                                            <div class="col-12 py-3 px-4">
                                                <div class="position-relative w-100 bg-secondary bg-opacity-10 rounded-pill" style="height: 8px; box-shadow: inset 2px 2px 5px rgba(0,0,0,0.1);">
                                                    <?php 
                                                    ...
                                                    ?>
                                                    <div class="bg-primary position-absolute rounded-pill h-100 transition" style="width: <?= $prog ?>; box-shadow: 0 0 10px var(--primary-color);"></div>
                                                    <div class="bg-white border border-primary border-2 rounded-circle position-absolute top-50 translate-middle shadow" style="left: <?= $prog ?>; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center;">
                                            ...
                                            <div class="col-sm-6 mt-4">
                                                <div class="text-muted smaller fw-bold mb-1 text-uppercase">Recipient Information</div>
                                                <div class="fw-bold"><?= escape($shipment['recipient_name']) ?></div>
                                                <div class="small fw-medium text-muted"><?= escape($shipment['recipient_phone']) ?></div>
                                            </div>
                                            <div class="col-sm-6 mt-4 text-sm-end">
                                                <div class="text-muted smaller fw-bold mb-1 text-uppercase">Delivery Address</div>
                                                <div class="fw-bold small"><?= nl2br(escape($shipment['recipient_address'])) ?></div>
                                            </div>
                                            
                                            <div class="col-sm-6 mt-4">
                                                <div class="text-muted smaller fw-bold mb-1 text-uppercase">Courier / Logistics</div>
                                                <div class="fw-bold text-primary"><i class="bi bi-building me-1"></i><?= escape($shipment['agent_name'] ?? 'ConsignX HQ') ?></div>
                                            </div>
                                            <div class="col-sm-6 mt-4 text-sm-end">
                                                <div class="text-muted smaller fw-bold mb-1 text-uppercase">Booking Date</div>
                                                <div class="fw-bold"><?= date('M d, Y', strtotime($shipment['created_at'])) ?></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-5">
                                    <div class="neumorphic-card p-4 h-100">
                                        <h5 class="fw-bold mb-4 pb-3 border-bottom border-secondary border-opacity-10"><i class="bi bi-geo me-2 text-primary"></i>Status Timeline</h5>
                                        <div class="ps-2">
                                            <ul class="timeline">
                                                <?php foreach ($history as $item): ?>
                                                    <li class="timeline-item">
                                                        <div class="fw-bold text-primary mb-0"><?= escape($item['status']) ?></div>
                                                        <div class="smaller text-muted fw-bold mb-2"><?= date('M d, Y - h:i A', strtotime($item['created_at'])) ?></div>
                                                        
                                                        <?php if (!empty($item['location'])): ?>
                                                            <div class="small fw-bold"><i class="bi bi-geo-alt me-1 text-muted"></i><?= escape($item['location']) ?></div>
                                                        <?php endif; ?>
                                                        
                                                        <?php if (!empty($item['remarks'])): ?>
                                                            <div class="small text-muted fst-italic py-2 px-3 bg-primary bg-opacity-5 rounded mt-2 border-start border-3 border-primary">
                                                                "<?= escape($item['remarks']) ?>"
                                                            </div>
                                                        <?php endif; ?>
                                                    </li>
