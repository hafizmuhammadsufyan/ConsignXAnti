 ...
 * 
 * Expected variables:
 * @param string $role 'admin', 'agent', or 'customer'
 * @param string $active_page The filename of the active page (e.g., 'dashboard.php')
 */
...
<nav class="sidebar d-flex flex-column justify-content-between neumorphic-card border-0 shadow-lg">
    ...
    <div>
        ...
        <ul class="nav flex-column gap-2 mt-4">
            <?php foreach ($menu_items as $item): ?>
                <li class="nav-item">
                    <a class="nav-link neumorphic-btn <?= ($active === $item['link'] || $active === $item['link']) ? 'btn-primary text-white active' : 'text-decoration-none' ?> text-center"
                        href="<?= $item['link'] ?>">
    ...
    <div class="mt-auto pt-3 border-top border-secondary border-opacity-10">
        <div class="px-2 mb-3">
            <a href="../shared/profile.php" class="nav-link neumorphic-btn <?= ($active === 'profile.php') ? 'btn-primary text-white active' : 'text-decoration-none text-muted' ?> py-2 text-center w-100">
        ...
        <a href="../auth/logout.php" class="btn neumorphic-btn btn-danger w-100 fw-bold logout-btn">
